<div class="form-group {{ $errors->has($input['name']) ? 'has-error' : '' }}">
    ...
    @elseif ($input['input'] === 'password')
        <input type="password" class="form-control" id="{{ $input['name'] }}" name="{{ $input['name'] }}" value="{{ old($input['name'], $input['value']) }}" @if ($input['required']) required @endif>
    @else
