...
var back = (function () {
    ...
    var appoint = function(url, that, errorAjax) {
        var urlActive = '/admin/faculties/appointment/' + that.val()
        if(that.is(':checked')) {
            urlActive += '/1'
        }
        // If "active" is checked we must reload the page
        if(getCheckboxValueByName('appointment')) {
            ajax(urlActive, 'PUT', url, errorAjax)
        } else {
            ajaxNoLoad(urlActive, 'PUT', errorAjax)
        }
    ...
    var search = function (url, that, errorAjax) {

            var urlActive = url + '?new=on&order=created_at&direction=desc&search='+that.val()
            ajax(urlActive, 'GET',urlActive, errorAjax)
