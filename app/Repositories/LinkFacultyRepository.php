...
class LinkFacultyRepository
{
     ...
     */
    public function __construct(LinkFaculty $link_faculty, Tag $tag, Comment $comment)
    {
        $this->model = $link_faculty;
        $this->tag = $tag;
    ...
    public function store($request)
    {
        // dd($request->all());
            ...
            $record_exists = LinkFaculty::where('event_id',$request->event_id)->where('faculty_id',$request->faculty_id)->first();

            if(empty($record_exists))
            {    
                    $request->merge(['user_id' => auth()->id()]);
                    $request->merge(['active' => $request->has('active')]);
                    $link_faculty = LinkFaculty::create($request->all());
     ...
     * @return void
     */
    public function update($link_faculty, $request)
    {
        // dd($request->all());
            $record_already_exists = LinkFaculty::where('id','!=',$link_faculty->id)->where('faculty_id','=',$request->faculty_id)->where('event_id','=',$request->event_id)->whereNull('deleted_at')->first();
           ...
           //dd($record_already_exists);
            ...
            {   
                    $request->merge(['active' => $request->has('active')]);
                    $link_faculty->update($request->all());
