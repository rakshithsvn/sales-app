...
class ProductRepository
{
     ...
     */
    public function __construct(Product $post, Tag $tag, Comment $comment)
    {
        $this->model = $post;
        $this->tag = $tag;
    ...
    public function store($request)
    {
        ...
        if (!empty($request->product_date)) {
            $product_Date = Carbon::createFromFormat('d/m/Y', $request->product_date)->format('Y-m-d');
        } else {
            $product_Date = null;
        }
        ...
        if (($news_id->id != $sub_menu_id) && ($case_study_id->id != $sub_menu_id)) {
            ...
            if ($record_exists == null) {
                if (isset($request->tab_section)) {
                    ...
                    $request->merge(['product_date' => $product_Date]);
