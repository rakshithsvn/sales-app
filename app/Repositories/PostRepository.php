...
class PostRepository
{
    ...
    public function store($request)
    {
        ...
        // $product_id = ParentMenu::select('id')->where('slug','=','services')->first();
    ...
    public function storeCount($request)
    {
       // dd($request->all());
        $request->merge(['user_id' => auth()->id()]);

        $choose = ChooseNo::Find($request->id);

        if($choose){
             $choose->update($request->all());
        }
        else{
            $choose = ChooseNo::create($request->all());
        }

    }

    public function storeROI($request)
    {
        // dd($request->all());

        $request->merge(['user_id' => auth()->id()]);

        $roi = DB::table('roi_creds')->Find($request->id);

        if($roi){
            DB::table('roi_creds')->update($request->except('_token'));
        }
        else{
            $roi = DB::table('roi_creds')->insert($request->except('_token'));
        }
