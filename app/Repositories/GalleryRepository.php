...
class GalleryRepository
{
    ...
    public function store($request)
    {
        // dd($request->all());
            ...
            $record_exists = GalleryTab::where('event_tab_id',$request->event_tab_id)->first();
            ...
            if(empty($record_exists))
            {
                if(isset($request->tab_section))
                {
                    foreach ($request->tab_title as $key => $value) {
                        ...
                        $project_tab->event_id = $request->event_id;
                        $project_tab->event_tab_id = $request->event_tab_id;
    ...
    public function update($project, $request)
    {
        ...
        //$record_already_exists = GalleryTab::where('event_tab_id','!=',$request->event_tab_id)->where('tab_title','=',$request->tab_title)->whereNull('deleted_at')->first();
                ...
                foreach ($request->tab_title as $key => $value) {
                    ...
                    if (is_array($value)){
                        foreach ($value as $tab_key => $tab_value) {
                            GalleryTab::where('id', $key)->update(['event_id' => $request->event_id, 'event_tab_id' => $request->event_tab_id, 'tab_title' => $tab_value,'tab_time' => $request->tab_time[$key][$tab_key],'speaker_id' => $request->speaker_id[$key][$tab_key] ]);
                        }
