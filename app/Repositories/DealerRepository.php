...
class DealerRepository
{
     ...
     */
    public function __construct(Dealer $post, Tag $tag, Comment $comment)
    {
        $this->model = $post;
        $this->tag = $tag;
    ...
    public function store($request)
    {
        ...
        if (!empty($request->dealer_date)) {
            $dealer_Date = Carbon::createFromFormat('d/m/Y', $request->dealer_date)->format('Y-m-d');
        } else {
            $dealer_Date = null;
        }
        ...
        // $dealer_id = ParentMenu::select('id')->where('slug','=','services')->first();
        ...
        if (($news_id->id != $sub_menu_id) && ($case_study_id->id != $sub_menu_id)) {
            ...
            if ($record_exists == null) {
                if (isset($request->tab_section)) {
                    ...
                    $request->merge(['dealer_date' => $dealer_Date]);
