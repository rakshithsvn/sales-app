...
class AdminController extends Controller
{
    ...
    public function getLogView(){
         $activitys = null;
         return view('back.activityLog', compact('activitys'));
    }

    public function getAjaxLogView(Request $request){

        $activitys = Activity::orderBy('id','desc')->limit(100)->get();

        $activitys = $activitys->map(function ($item, $key) {
            return ['id'=>$item->id,'subject_id'=>@$item->post_table->title,'description'=>$item->description,'causer_id'=>@$item->user->name,'created_at'=>$item->created_at->format('d-m-Y'),'properties'=>json_encode(collect(json_decode($item->properties)->attributes)->except(['body']))];
            // return ['id'=>$item->id,'subject_id'=>@$item->post_table->title,'description'=>$item->description,'causer_id'=>@$item->user->name,'created_at'=>$item->created_at->format('d-m-Y'),'action'=>'<a class="btn ">action</a>'];
        });

        return \DataTables::of($activitys)->rawColumns(['properties'])->make(true);
    }
